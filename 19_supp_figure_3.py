"""Supplementary Figure 3: maternal circuits captured but stable.
...
def build():
    ...
    for ext in ['png', 'svg']:
        fig.savefig(f'{out_dir}/figure_supp_3.{ext}', dpi=300,
                    facecolor='white')
    plt.close(fig)
    print(f'wrote {out_dir}/figure_supp_3.png/.svg ({W:.1f}x{H:.1f}in)')
